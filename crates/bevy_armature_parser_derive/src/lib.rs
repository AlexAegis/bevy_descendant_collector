...
pub fn armature_path_map(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
	...
	let assignments = fields
		...
		.map(|(ident, path)| {
			...
			quote! {
				#ident: bevy_armature_parser::find_named_entity::find_named_entity(armature_source_root, &named_query, &vec![#path]).unwrap_or_else(|| {
					let named_entity_paths = bevy_armature_parser::find_named_entity::collect_named_entity_paths(armature_source_root, &named_query);
	...
	proc_macro::TokenStream::from(quote! {
		impl #impl_generics bevy_armature_parser::armature_loader::ArmatureLoader for #struct_name #ty_generics #where_clause {
